...
public:
    ...
    // Returns the camera pose (empty if tracking fails).
    Sophus::SE3f TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp,vector<OutputParams>result, const vector<IMU::Point>& vImuMeas = vector<IMU::Point>(), string filename="");
